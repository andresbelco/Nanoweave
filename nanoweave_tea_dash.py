...
app.layout = html.Div(style={"background": C["bg"], "minHeight": "100vh",
    ...
    html.Div(style={"display":"flex","gap":"0","maxWidth":"1500px","margin":"0 auto","minWidth":"700px"}, children=[
        ...
        # ══ MAIN CONTENT ═══════════════════════════════════════════════════
        html.Div(style={"flex":"1","padding":"28px 28px","minWidth":"0"}, children=[
            ...
            html.Div(style={"display":"grid","gridTemplateColumns":"repeat(auto-fit, minmax(170px, 1fr))","gap":"12px",
            ...
            html.Div(style={"display":"grid","gridTemplateColumns":"repeat(auto-fit, minmax(280px, 1fr))","gap":"16px",
                            "marginBottom":"24px"}, children=[
            ...
            html.Div(style={"display":"grid","gridTemplateColumns":"repeat(auto-fit, minmax(130px, 1fr))","gap":"12px",
                            "marginBottom":"24px"}, children=[
            ...
            # KPI summary row
            html.Div(style={"display":"grid","gridTemplateColumns":"repeat(auto-fit, minmax(150px, 1fr))","gap":"12px",
                            "marginBottom":"16px"}, children=[
            ...
            html.Div(style={"display":"grid","gridTemplateColumns":"repeat(auto-fit, minmax(280px, 1fr))","gap":"16px",
                ...
                html.Div([
                    html.Div("Franchisee Buy Price & Developer Margin per Scenario",
                             style={**LABEL_STYLE,"marginBottom":"10px"}),
                    html.Div(id="fr-price-table", style={"border":"1px solid #ccc","borderRadius":"4px","padding":"8px"}),
                ], style=CARD_STYLE),
            ]),
...
def update_all(biomass_total, biomass_key, power_mode, elec_price,
    ...
    fig_opex.update_layout(
        ...
        legend=dict(orientation="h",font_size=9,bgcolor="rgba(0,0,0,0)",y=-5),
...
def update_franchise(
    ...
    modules_per_fr = modules_per_fr or 4
    ...
    # Franchisee revenue = cellulose revenue (buy_price × cell_yr) + fertilizer revenue
    # Franchisees keep ALL fertilizer output — valued at fert_price ($/t N-equiv.)
    # Franchisee EBITDA = total revenue − total OPEX (incl. lease)
    # License = 5% × franchisee EBITDA
    # Developer trade margin = (sell_price − buy_price) × cell_yr
    # Developer net/mod = trade margin + license
    fert_rev_per_mod = r1["rev_fert"]   # franchisee keeps all fertilizer revenue
    ...
    for label, s in scenarios.items():
        ...
        fr_cell_rev_per_mod = buy_price_per_t * cell_yr_per_mod
        fr_revenue_per_mod  = fr_cell_rev_per_mod + fert_rev_per_mod
        ...
        s["fr_cell_rev_per_mod"]      = fr_cell_rev_per_mod
        ...
        s["fr_total_cell_rev"]        = tot_mods * fr_cell_rev_per_mod
        s["fr_total_fert_rev"]        = tot_mods * fert_rev_per_mod
    ...
    TH = {"fontSize":"10px","color":C["text2"],"fontFamily":"monospace",
          "padding":"6px 10px","borderBottom":f"1px solid {C['border']}",
          "textAlign":"left","letterSpacing":"0.8px","whiteSpace":"nowrap"}
    TD = lambda v,col=C["text"]: {"fontSize":"11px","fontFamily":"monospace",
          "color":col,"padding":"7px 10px",
          "borderBottom":f"1px solid rgba(30,46,36,0.4)"}
    TDB = lambda v,col=C["text"]: {"fontSize":"11px","fontFamily":"monospace",
          "color":col,"padding":"7px 10px","fontWeight":"700",
          "borderBottom":f"1px solid rgba(30,46,36,0.4)"}
    ...
    h1 = html.Tr([
        ...
        html.Th("─── FRANCHISEE P&L (per module/yr) ───",
                colSpan=5, style={**TH,"textAlign":"center",
    ...
    h2 = html.Tr([
        ...
        html.Th("Cell Rev.",        style={**TH,"borderLeft":f"1px solid {C['border']}"}),
        html.Th("Fert. Rev.",       style=TH),
    ...
    for label, s in scenarios.items():
        ...
        data_rows.append(html.Tr([
            ...
            html.Td(fmt_usd(s["fr_cell_rev_per_mod"]),     style={**TD(0),"borderLeft":f"1px solid {C['border']}"}),
            html.Td(fmt_usd(fert_rev_per_mod),             style={**TD(0),"color":C["amber"]}),
    ...
    data_rows.append(html.Tr([
        ...
        html.Td(fmt_usd(mid["fr_total_cell_rev"]),     style={**TDB(0,C["green"]),"borderLeft":f"1px solid {C['border']}"}),
        html.Td(fmt_usd(mid["fr_total_fert_rev"]),     style=TDB(0,C["amber"])),
    ...
    price_tbl = html.Div([
        html.Div([
            ...
            html.Span(f"  (CAPEX {fmt_usd(fr_capex_per_mod)} ÷ {lifespan} yrs)",
                      style={"color":C["text2"]}),
            html.Span(f"  ·  Fert. rev./mod: ", style={"color":C["text2"]}),
            html.Span(fmt_usd(fert_rev_per_mod)+"/yr", style={"color":C["amber"],"fontWeight":"700"}),
            html.Span(f"  ·  License: 5% franchisee EBITDA (cell + fert, after lease)",
                      style={"color":C["text2"]}),
...
def generate_pdf_report(
    ...
    # Franchisees keep ALL fertilizer revenue (on top of cellulose buy price)
    # EBITDA = (cellulose rev + fert rev) − total OPEX (after lease cost)
    ...
    for lbl, mp in [("+10%", 0.10), ("+15%", 0.15), ("+20%", 0.20)]:
        ...
        tot_mods   = n_franchisees * modules_per_fr
        fr_sc[lbl] = dict(
            buy_t=buy_t, cell_rev=cell_rev, fert_rev=fert_rev_mod,
            fr_rev=fr_rev, fr_ebitda=fr_ebitda,
            fr_marg=fr_marg, lic=lic, trade=trade, dev_inc=dev_inc,
            fr_opex_per_mod=fr_total_opex_y,
            tot_fr_cell_rev=tot_mods*cell_rev, tot_fr_fert_rev=tot_mods*fert_rev_mod,
    ...
    cover_rows = [
        ...
        [_P("Plasma Biorefinery  ·  Modular system  · Cellulose NBSK pulp quality scenario",
    ...
    s.append(_P(
        ...
        f"<b>Franchisees keep all fertilizer output</b>: {fmt_usd(fert_rev_mod)}/module/yr additional revenue. "
        f"Developer buys at <b>total OPEX + margin</b>, resells at "
        f"<b>${sell_price}/t</b>. "
        f"<b>License: 5% of franchisee EBITDA</b> (cellulose + fertilizer, after lease) per module per year. "
    ...
    fr_pldata = [
        ["Scenario", "Buy $/t",
         "Cell Rev.", "Fert. Rev.", "Fr. OPEX", "Fr. EBITDA", "Fr. Margin",
    ...
    s.append(_dt_row(fr_pldata,
                     [CW*0.08, CW*0.08, CW*0.10, CW*0.10, CW*0.10, CW*0.10,
                      CW*0.08, CW*0.10, CW*0.10, CW*0.16]))
    s.append(_Sp(1, 6))
    s.append(_P(
        ...
        f"Franchisees keep all fertilizer output: {fmt_usd(fert_rev_mod)}/module/yr. "
    ...
    tot_mods = n_franchisees * modules_per_fr
    port_d = [["Scenario", "Fr. Cell Rev.", "Fr. Fert Rev.", "Fr. Total OPEX", "Fr. Total EBITDA",
               "Dev Total Trade", "Dev Total License", "Dev Gross Income"]]
    for lbl, sc in fr_sc.items():
        port_d.append([
            lbl,
            fmt_usd(sc["tot_fr_cell_rev"]),
            fmt_usd(sc["tot_fr_fert_rev"]),
    ...
    s.append(_dt_row(port_d,
                     [CW*0.10, CW*0.13, CW*0.13, CW*0.13, CW*0.13, CW*0.13, CW*0.12, CW*0.13]))
    s.append(_Sp(1, 10))
